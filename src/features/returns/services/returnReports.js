...
export function buildReturnExportRows(rows, type) {
  return rows.flatMap((row) => {
    ...
    return items.map((item) => ({
      "Jenis Retur": type === "customer" ? "Konsumen" : "Supplier",
      "No Retur": row.no_retur,
      Tanggal: formatDateTime(row.created_at, { dateStyle: "medium", timeStyle: "short" }),
      Status: getStatusLabel(row.status),
      Pihak: type === "customer" ? row.customer_name || "-" : row.supplier_name || "-",
      "Transaksi Asal": type === "customer" ? row.transaction_no || "-" : "-",
      Produk: item.product_name || "-",
      "Kode Produk": item.product_code || "-",
      Kategori: item.category || "-",
      Qty: Number(item.quantity || 0),
      "Harga/Modal Satuan": Number(type === "customer" ? item.unit_price || 0 : item.unit_cost || 0),
      Subtotal: Number(type === "customer" ? item.subtotal_refund || 0 : item.subtotal_cost || 0),
      Alasan: getReasonLabel(row.reason),
      Kondisi: item.condition || row.condition || "-",
      Catatan: item.notes || row.notes || "-",
      Restock: type === "customer" ? (row.restock ? "Ya" : "Tidak") : "-",
...
export async function exportReturnWorkbook({ supplierRows, customerRows, fileName }) {
  ...
  addJsonSheet(workbook, "Detail Retur", allRows);
